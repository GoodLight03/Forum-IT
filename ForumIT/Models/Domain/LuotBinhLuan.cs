...
{
    // Đọc giá trị của cookie và tăng giá trị đó lên 1
    visitCount = int.Parse(Request.Cookies["VisitCount"]) + 1;
}

// Thiết lập lại giá trị của cookie
Response.Cookies.Append("VisitCount", visitCount.ToString());

// Truyền giá trị visitCount vào view để hiển thị
...
1) DataType
Thuộc tính này được sử dụng để xác định kiểu dữ liệu của mô hình.

Syntax
[DataType(DataType.Text)]

2) Required
Thuộc tính này xác định giá trị cụ thể là bắt buộc đối với một yêu cầu nhất định
Syntax
[Required(ErrorMessage="Please enter your name"),MaxLength(50)]

3) StringLength
Sử dụng thuộc tính chú thích này, chúng ta có thể đặt phạm vi chuỗi tối đa và tối thiểu của thuộc tính.
Syntax
[StringLength(50,ErrorMessage="Please do not enter values over 50 characters")]

4) MaxLength
Với thuộc tính chú thích này, chúng ta có thể đặt độ dài tối đa của thuộc tính.
Syntax
[MaxLength(5)]

5) Range
Chúng ta có thể áp dụng thuộc tính chú thích Phạm vi để xác định phạm vi giữa hai số.
Syntax
[Range(50,250,ErrorMessage="Please enter the correct value")]

6)Bind
Thuộc tính chú thích này xác định các trường cần nhập hoặc loại bỏ để liên kết mô hình.
Syntax
[Bind(Exclude = "EmployeeID")]

7) DisplayFormat
Thuộc tính chú thích này cho phép chúng tôi đặt định dạng ngày được xác định theo thuộc tính.
Syntax
[DisplayFormat(DataFormatString = "{0:MM.DD.YYYY}")]

8) DisplayName
Với thuộc tính chú thích này, chúng ta có thể đặt tên thuộc tính sẽ hiển thị ở dạng xem.
Syntax
[Display(Name="Employee Name")]

9) RegularExpression
Sử dụng thuộc tính này, chúng ta có thể đặt mẫu biểu thức chính quy (biểu thức chính quy) cho thuộc tính. Ví dụ: ID email.
Syntax
[RegularExpression(@"^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$", ErrorMessage = "Your Email is not valid.")]

10) ScaffoldColumn
Chỉ định xem giàn giáo được sử dụng bởi một lớp hay một cột dữ liệu.
...
1. System.ComponentModel.DataAnnotations kết hợp các thuộc tính tiếp theo ảnh hưởng và kiểm tra kích thước hoặc tính chất rỗng của cột.

MinLength Độ dài nhỏ nhất
MaxLength Độ dài tối đa
StringLength Chiều dài chuỗi
Key Chìa khóa
Timestamp Dấu thời gian
ConcurrencyCheck Kiểm tra đồng thời
Required Yêu cầu


2. Không gian tên System.ComponentModel.DataAnnotations.Schema bao gồm các thuộc tính tiếp theo định hình lại lược đồ của cơ sở dữ liệu.
Index Mục lục
ForeignKey Khóa ngoại
NotMapped Chưa được ánh xạ
InverseProperty Thuộc tính nghịch đảo
Table Bàn
Column Cột

3. System.ComponentModel.DataAnnotations assembly which has several built-in validation types of attributes’ properties which we can implement as per the code logic.
...
Số điện thoại
Mã bưu điện
Thuộc tính xác thực tích hợp

3.Trong ASP.NET MVC, chúng ta có thể coi các thuộc tính xác thực tích hợp là xác thực tùy chỉnh. Dưới đây là một vài ví dụ:
...
So sánh: Nó xác nhận và khớp các thuộc tính của hai trường trong một mô hình.
Phạm vi: Nó xác nhận khi một giá trị thuộc tính nhất định nằm trong một phạm vi nhất định.
RegularExpression: Nó xác nhận khi giá trị thuộc tính đáp ứng một biểu thức chính quy nhất định.
Từ xa: Nó hoạt động khi xác thực từ xa; chúng ta có thể gọi một hàm phương thức hành động trên máy chủ và xác thực các đầu vào cụ thể ở phía máy khách.
DevExtremeRequired: Nó xác nhận khi giá trị thuộc tính boolean nhất định là đúng hoặc sai.
Tùy chỉnh: Nó cho phép bạn xác định thuộc tính xác thực tùy chỉnh.

---------------------------Biểu thức chính quy
Biểu thức chính quy (regular expression) là một chuỗi ký tự đặc biệt được sử dụng để tìm kiếm và so khớp các mẫu trong chuỗi văn bản. Trong C#, bạn có thể sử dụng biểu thức chính quy để kiểm tra tính hợp lệ của các chuỗi, tìm kiếm và thay thế các chuỗi, và thực hiện các thao tác khác liên quan đến xử lý chuỗi.

Dưới đây là một số cú pháp và ký hiệu phổ biến trong biểu thức chính quy:

Ký tự đại diện:

\d: Chữ số từ 0 đến 9.
\w: Ký tự chữ, bao gồm chữ cái từ A đến Z (viết hoa và viết thường), chữ số từ 0 đến 9 và dấu gạch dưới (_).
\s: Khoảng trắng, bao gồm dấu cách, tab và xuống dòng.
Ký tự lặp:
...
|: Tìm kiếm một trong các mẫu được cung cấp.
Ký tự escape:
